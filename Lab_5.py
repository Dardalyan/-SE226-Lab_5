...
#TASK 2
def rep_with_upper(param:str):
    length = len(param)
    random_choose= randint(0,length)
    char = param[random_choose]
    param = param.replace(char,char.upper())
    return param


def swap_letters(param:str):
    temp = []
    for i in param:
        temp.append(i)
    charFT=temp[0]+temp[1]
    charLT=temp[len(temp)-2]+temp[len(temp)-1]
    newStr=charLT
...
initial_password = passGen(random_words)
print('Initial Password: ',initial_password)

temp=rep_with_upper(initial_password)
final_password=swap_letters(temp)

print('Final Password: ',final_password)

#TASK 3
def search_letter(param1:str,param2:str):
    if param2 in param1:
        return True
    else:return False
print(search_letter(final_password,'b'))